...
$(function() {
  ...
  for(var i=1; i<=5; i++) {
    photoFrame.find(".imageArea").append("<img src='images/gakka" + i + ".jpg'>");
  }

  $("#next").on("click", function() {
    toDisabled();
    photoFrame.find(".imageArea").animate(
      {"left" : "-=300px"}, "fast", "linear", next);
  });

  $("#prev").on("click", function() {
    toDisabled();
    photoFrame.find(".imageArea").animate(
      {"left" : "+=300px"}, "fast", "linear", prev);
  });
});
...
function next() {
  ...
  if(parseInt($("#imageArea" + currentIndex).css("left")) == -900) {
    // 末尾の画像を表示したときの処理
    // 隠れている方のimageAreaのleftプロパティをphotoFrameの右側に設定
    $("#imageArea" + otherIndex).css("left", "300px");
  }
  else if(parseInt($("#imageArea" + currentIndex).css("left")) == -1200) {
...
function prev() {
  ...
  if(parseInt($("#imageArea" + currentIndex).css("left")) == 0) {
    ...
    $("#imageArea" + otherIndex).css("left", "-1200px");
  }
  else if(parseInt($("#imageArea" + currentIndex).css("left")) == 300) {
    // imageAreaが切り替わったタイミングでカレントインデックスを変更
    currentIndex = otherIndex;
  }	
  toEnabled();
